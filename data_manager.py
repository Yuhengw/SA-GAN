...
class TrainDataset(data.Dataset):
    ...
    def __init__(self, config):
        ...
        if not os.path.exists(train_list_file) or os.path.getsize(train_list_file) == 0:
            #print (os.listdir(os.path.join(config.datasets_dir, 'imag_4')))
    ...
    def __getitem__(self, index):
        ...
        M = np.clip(((x + 0.001) * 50).sum(axis=2) / 255, 0, 1).astype(np.float32)
...
class TestDataset(data.Dataset):
    def __init__(self, test_dir, in_ch, out_ch):
        ...
        #self.test_files = np.loadtxt(('data/skin_oct/test_list.txt'),oct_imag_4_spa_ssim_yes_attention_yes_with_weight
