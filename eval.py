...
def test_generated(config, test_data_loader, criterionMSE, epoch):
    avg_mse = 0
    avg_psnr = 0
    avg_ssim = 0
    max_mes = 0
    ...
    max_ssim = 0
    for i, batch in enumerate(test_data_loader):
        x, t, ig = Variable(batch[0]), Variable(batch[1]),Variable(batch[2])
        name = test_data_loader.dataset.imlist[i]
        if config.cuda:
            x = x.cuda(0)
            t = t.cuda(0)
            ig = ig.cuda(0)
        ...
        mse = criterionMSE(out, t)
        psnr = 10 * np.log10(1 / (mse.item()+0.0001))
        #img1 = np.tensordot(out.cpu().numpy()[0, :3].transpose(1, 2, 0), [0.298912, 0.586611, 0.114478], axes=1)
        #img2 = np.tensordot(t.cpu().numpy()[0, :3].transpose(1, 2, 0), [0.298912, 0.586611, 0.114478], axes=1)
        img1 = np.tensordot(out.cpu().numpy()[0, :3].transpose(1, 2, 0), [0.298912, 0.586611, 0.114478], axes=1)
        img2 = np.tensordot(t.cpu().numpy()[0, :3].transpose(1, 2, 0), [0.298912, 0.586611, 0.114478], axes=1)
        ssim = SSIM(img1, img2)
        '''if max_ssim <=ssim <0.80:
            max_ssim = ssim
            max_mse = mse
            max_psnr = psnr
            name_target = name'''
    ...
    print("===> Avg. PSNR: {:.4f} dB".format(avg_psnr))
    print("===> Avg. SSIM: {:.4f} dB".format(avg_ssim))
    #print("===> Max. MSE: {:.4f}".format(max_mse))
